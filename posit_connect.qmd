...
First access RStudio from Dash Platform on Databricks / Workbench (link to be provided at mid-end of October as licenses will be made available by then) which will replace the RStudio within Databricks).
...
#### Method A: Direct Deployment from your current IDE
...
You need an API key to be created within [new Connect Server](https://dash-connect-prd.azure.defra.cloud/). See here [API key] how to create an API key.
...
7.  I do not have manifest.json file therefore cannot migrate the app and will follow the 8th step. If you do have manifest.json file then app should be migrated on new Connect. If you do not have manifest.json follow the steps here - [Writing Manifest json in R and Python]

```{r, echo=FALSE, out.width = '200%'}
knitr::include_graphics("images/git_migration_quarto_6.png")
```

8.  The app was migrated on the new Connect as it can be seen in the picture bellow.
...
1.  is the old connect server - <https://dap-prd2-connect.azure.defra.cloud/>
2.  and here is the new connect server - [https://dash-connect-prd.azure.defra.cloud](https://dash-connect-prd.azure.defra.cloud/connect)/
...
#### Method D - Download - Upload bundle
...
1.  Go to old connect **-** <https://dap-prd2-connect.azure.defra.cloud/>

2.  Go onto your app amd download the bundle. Just like in the picture

    ```{r, echo=FALSE, out.width = '200%'}
    knitr::include_graphics("images/download_bundle.png", dpi = NA )
    ```

3.  Upload it onto any IDE (RStudio/VS Code) or Workbench

4.  For next steps, follow sections [Publishing from RStudio] & [Publishing from Python].
...
#### Writing Manifest json in R and Python
