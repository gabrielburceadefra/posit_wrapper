...
file <- db_volume_read('/Volumes/tst1_dash_lab/soundscapes_restricted/shared_external_volume/penguins.csv', tempfile(), perform_request = TRUE)   
...
``` r
db_volume_write('/Volumes/tst1_dash_lab/soundscapes_restricted/shared_external_volume/gentoo.csv','gentoo.csv', overwrite = TRUE, perform_request = TRUE)
```
...
```{r  echo=FALSE, out.width = '100%'}
knitr::include_graphics("images/workbench_rstudio_versions.png")
```
...
# Open file to upload  
adelie = df[df['species'] == 'Adelie']  adelie.to_csv("/tmp/adelie.csv", index=False)
```
...
with open("/tmp/adelie.csv", "rb") as f: csv = f.read()     
...
20. Press Open and as signed into Github a Device Activation will come up.0
